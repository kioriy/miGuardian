...
class StoreScreen(Screen):
    ...
    def load_breakfast_records(self):
        ...
        for record in breakfast_records:
            ...
            list_item.add_widget(MDListItemLeadingAvatar(source=f"fotos/{nnombre} {napellidos}.jpeg"))
            list_item.add_widget(MDListItemHeadlineText(text=f"{record.nombre} {record.apellidos}"))
            list_item.add_widget(MDListItemSupportingText(text=f"{record.grado} - {record.grupo}"))
            list_item.add_widget(MDListItemTertiaryText(text=str(f"{record.date.strftime('%d-%m-%y')} {record.time}")))
...
class MiGuardianApp(MDApp):
    def build(self):
        ...
        self.title = "mi Guardian v1.11.5"
        #db.updatedb()
    ...
    def show_student_info(self):
        ...
        if self.es_url(main_screen.ids.barcode_input.text):
            barcode = urlparse(main_screen.ids.barcode_input.text).fragment
        else:
    ...
    def switch_screen(self):
        if self.screen_status == "HDMI":
            ...
            self.dialog = MDDialog(title='Confirma cambio de pantalla',
                                ...
                                buttons=[MDButton(text='Revertir',
                                                        on_release=self.dialog_close_revert),
                                            MDButton(text='Aceptar',
